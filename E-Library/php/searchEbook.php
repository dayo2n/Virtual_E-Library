...
<body>
    <div class="container">
        ...
        <form class="row" action="searchEbook.php" method="get">
            <div class="col-auto">
        ...
        <table class="table table-bordered text-center">
            ...
            while($row = $stmt -> fetch(PDO::FETCH_ASSOC)){
                ...
                <tr>
                    ...
                    <td><button class="btn btn_on_search" name="block" disabled>예약하기</button></td>
                    <?php
                        }else{ // 다른 사람이 이미 대출했다면 예약 가능
                            if(($row['CNT'] >= 3) or ($row['CNT_RESERVE'] >= 3)){ // 근데 이미 세 권 이상 대출했거나 예약했다면 예약 불가
                        ?>
                    <td class="isRented">대출 중</td>
                    <td><button class="btn btn_on_search" name="block" disabled>예약하기</button></td>
                    <?php
                    ...
                    <form action="bookDetail.php" method="get">
                        ...
                        <td><button type="submit" class="btn btn_on_search" name="reserve">예약하기</button></td>
                    </form>
                    <?php
                        }
                    }}else{ // 놀고있으면 대출가능
                        if($row['CNT'] >= 3){ // 근데 이미 세 권 이상 대출했으면 대출 불가
                            ?>
                    <td class="enableRent">대출 가능</td>
                    <td><button class="btn btn_on_search" name="block" disabled>대출하기</button></td>
                    <?php
                                }else{ // 아니면 예약 가능
                        ?>
                    <form action="bookDetail.php" method="get">
                        ...
                        <td><button type="submit" class="btn btn_on_search" name="rent">대출하기</button></td>
                    </form>
                    <?php 
                    }
                }
